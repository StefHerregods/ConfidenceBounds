...
// [[Rcpp::export]]
NumericMatrix DDM_confidence_bounds(double v, double a, double ter, double z, int ntrials, double s, double dt, double a2, double postdriftmod_1, double postdriftmod_2, double a2_slope, double ter2) {
  ...
  for (int i = 0; i < ntrials; i++) {
    ...
    double v_post_1 = v * postdriftmod_1;
    double v_post_2 = v * postdriftmod_2;
    if (resp == 1){
      while ((evidence < a + a2/2 - t2*a2_slope) && (evidence > a - a2/2 + t2*a2_slope)){
        t2 = t2 + dt;
        evidence = evidence + v_post_1 * dt + s * sqrt(dt) * zigg.norm();
      ...
      while ((evidence < a2/2 - t2*a2_slope) && (evidence > -a2/2 + t2*a2_slope)){
        t2 = t2 + dt;
        evidence = evidence + v_post_2 * dt + s * sqrt(dt) * zigg.norm();
