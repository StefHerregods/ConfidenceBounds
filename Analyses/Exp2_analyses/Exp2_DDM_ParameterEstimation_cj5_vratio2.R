...
itermax <- 10  # Number of DeOptim iterations
...
chi_square_optim <- function(params, all_observations, returnFit){  
  ...
  for (i in 1:3){
    ...
    predictions$cj_5 <- cut(predictions$conf_evidence, breaks=c(-Inf, 1.5 * params['a2'] / 6, 3 * params['a2'] / 6, 4 * params['a2'] / 6, 5 * params['a2'] / 6, Inf), labels = c(2, 3, 4, 5, 6))
    ...
    if(returnFit==0){ 
      ...
      conf_obs_props_1 <- c(conf_obs_2_proportion, conf_obs_3_proportion, conf_obs_4_proportion, conf_obs_5_proportion, conf_obs_6_proportion)
      ...
      pred_props_rtconf_1 <- c(conf_2_pred_proportion, conf_3_pred_proportion, conf_4_pred_proportion, conf_5_pred_proportion, conf_6_pred_proportion)
  ...
  for(c in 1:4){  # For each condition separately 
    ...
    # Else, estimate parameters
      ...
      optimal_params <- DEoptim(chi_square_optim,  # Function to optimize
                                ...
                                lower = c(0, 0, 0, .5,   0, 0.0001, 0,  0,  0, -2),  
