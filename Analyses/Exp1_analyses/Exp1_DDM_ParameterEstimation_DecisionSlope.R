...
chi_square_optim <- function(params, all_observations, returnFit){  
  ...
  names(params) <- c('v1', 'v2', 'v3', 'a', 'a_slope', 'ter', 'a2', 'postdriftmod', 'a2_slope', 'ter2')
  ...
  for (i in 1:3){
    ...
    predictions <- data.frame(DDM_confidence_bounds(v = params[v_vector[i]], a = params['a'], a_slope = params['a_slope'], ter = params['ter'], z = z, ntrials = ntrials, s = sigma, dt = dt, a2 = params['a2'], postdriftmod = params['postdriftmod'], a2_slope = params['a2_slope'], ter2 = params['ter2']))
...
for(i in 1:40){  # For each participant separately
  ...
  for(c in 1:4){  # For each condition separately 
    ...
    file_name <- paste0('Results\\DecisionSlopeTest_results_sub_', i, '_', condLab[c], '.Rdata')
    ...
    # Else, estimate parameters
      ...
      optimal_params <- DEoptim(chi_square_optim,  # Function to optimize
                                # Possible values for v (for each level of coherence: 0.1, 0.2 and 0.4), a, a_slope, ter, a2, postdriftmod, a2_slope, ter2
                                lower = c(0, 0, 0, .3, 0,  0, 0.3, 0,  0, -2),  
                                upper = c(3, 3, 3,  4, 15, 1.5, 5.5, 15, 10, 2),
