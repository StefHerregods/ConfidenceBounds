...
// [[Rcpp::export]]
NumericMatrix DDM_confidence_bounds(double v, double a, double ter, double z, int ntrials, double s, double dt, double a2_upper, double a2_lower, double postdriftmod, double a2_slope_upper, double a2_slope_lower, double ter2) {
  ...
  for (int i = 0; i < ntrials; i++) {
    ...
    if (resp == 1){
      while ((evidence < a + a2_upper - t2*a2_slope_upper) && (evidence > a - a2_lower + t2*a2_slope_lower)){
        ...
        if (evidence >= a + a2_upper - t2*a2_slope_upper){
          DATA(i,6) = 1;
        } else if (evidence <= a - a2_lower + t2*a2_slope_lower){
          DATA(i,6) = 0;
          }
      }
    } else if (resp == -1){
      while ((evidence < a2_lower - t2*a2_slope_lower) && (evidence > - a2_upper + t2*a2_slope_upper)){
        t2 = t2 + dt;
        evidence = evidence + v_post * dt + s * sqrt(dt) * zigg.norm();
        if (evidence >= a2_lower - t2*a2_slope_lower){
          DATA(i,6) = 0;
        } else if (evidence <= - a2_upper + t2*a2_slope_upper){
          DATA(i,6) = 1;
        }
      }
    }
