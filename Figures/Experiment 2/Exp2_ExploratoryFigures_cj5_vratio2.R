...
data_viable <- data_viable[data_viable$cj > 1,]

# Simulating data based on estimated parameters

sourceCpp("C:\\Users\\herre\\OneDrive\\Documenten\\GitHub\\ConfidenceBounds\\Analyses\\Exp1_analyses\\DDM_confidence_bounds_vratio2.cpp") 

exclude <- c()

df <- data.frame(matrix(ncol = 12, nrow = 40*4))
colnames(df) <- c('sub', 'manipulation', 'v1', 'v2', 'v3', 'a', 'ter', 'a2', 'postdriftmod_1', 'postdriftmod_2', 'a2_slope', 'ter2')
condLab <- c('FastFast', 'AccFast', 'AccAcc', 'FastAcc') 
j <- 1
for (i in (21:40)){ 
  for(c in 1:4){
    file_name <- paste0('Parameter_estimation_cj5_vratio2\\exp2_cj5_vratio2_results_sub_', i, '_', condLab[c], '.Rdata')
    load(file_name)
    df[j,] <- c(i, condLab[c], results$optim$bestmem[1], results$optim$bestmem[2], results$optim$bestmem[3], results$optim$bestmem[4], results$optim$bestmem[5], results$optim$bestmem[6], results$optim$bestmem[7], results$optim$bestmem[8], results$optim$bestmem[9], results$optim$bestmem[10])
    j <- j + 1
  }
}
df[3:12] <- lapply(df[3:12], as.numeric)
...
for (i in (21:n)){
  for (j in 1:4){
    for (k in 1:3){
      ...
      v <- df_temp[[k + 2]]
      predictions_temp <- data.frame(DDM_confidence_bounds(v = v, a = df_temp$a, ter = df_temp$ter, z = z, ntrials = ntrials, s = sigma, dt = dt, a2 = df_temp$a2, postdriftmod_1 = df_temp$postdriftmod_1, postdriftmod_2 = df_temp$postdriftmod_2, a2_slope = df_temp$a2_slope, ter2 = df_temp$ter2))
      ...
      predictions_temp$cj <- cut(predictions_temp$conf_evidence, breaks=c(-Inf, 1.5 * df_temp$a2 / 6, 3 * df_temp$a2 / 6, 4 * df_temp$a2 / 6, 5 * df_temp$a2 / 6, Inf), labels = c(2, 3, 4, 5, 6))
      
      predictions <- rbind(predictions, predictions_temp)
    }
  }
}
predictions$cj <- as.numeric(levels(predictions$cj))[predictions$cj]
...
plot3 <- ggplot(data_viable[data_viable$cj == 1,], aes(x = coherence, y = cor)) + 
  stat_summary(aes(group = 1), fun.y = mean, colour = "black", geom = "line", size = 2) + 
  stat_summary(data = data_viable[data_viable$cj == 2,], aes(x = coherence, y = cor, group = 1), fun.y = mean, colour = "#202020", geom = "line", size = 2) + 
  stat_summary(data = data_viable[data_viable$cj == 3,], aes(x = coherence, y = cor, group = 1), fun.y = mean, colour = "#404040", geom = "line", size = 2) + 
  stat_summary(data = data_viable[data_viable$cj == 4,], aes(x = coherence, y = cor, group = 1), fun.y = mean, colour = "#606060", geom = "line", size = 2) + 
  stat_summary(data = data_viable[data_viable$cj == 5,], aes(x = coherence, y = cor, group = 1), fun.y = mean, colour = "#808080", geom = "line", size = 2) + 
  stat_summary(data = data_viable[data_viable$cj == 6,], aes(x = coherence, y = cor, group = 1), fun.y = mean, colour = "#909090", geom = "line", size = 2) + 
  xlab('Coherence') +
  ylab('% correct') +
  theme_bw()
