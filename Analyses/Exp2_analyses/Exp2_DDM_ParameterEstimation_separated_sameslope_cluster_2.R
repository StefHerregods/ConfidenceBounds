...
chi_square_optim <- function(params, all_observations, returnFit){  
  ...
  names(params) <- c('v1', 'v2', 'v3', 'a', 'ter', 'a2_upper', 'a2_lower', 'postdriftmod', 'a2_slope_upper', 'ter2')
  ...
  for (i in 1:3){
    ...
    predictions <- data.frame(DDM_confidence_bounds(v = params[v_vector[i]], a = params['a'], ter = params['ter'], z = z, ntrials = ntrials, s = sigma, dt = dt, a2_upper = params['a2_upper'], a2_lower = params['a2_lower'], postdriftmod = params['postdriftmod'], a2_slope_upper = params['a2_slope_upper'], a2_slope_lower = params['a2_slope_upper'], ter2 = params['ter2']))
...
for(c in 1:4){  # For each condition separately 
  ...
  file_name <- paste0('exp2_separated_2_results_sameslope_sub_', i, '_', condLab[c], '.Rdata')
  ...
  # Else, estimate parameters
    ...
    optimal_params <- DEoptim(chi_square_optim,  # Function to optimize
                              # Possible values for v (for each level of coherence: 0.1, 0.2 and 0.4), a, ter, a2_upper, a2_lower, postdriftmod, a2_slope_upper, ter2
                              lower = c(0, 0, 0, .5,   0, 0.0001, 0.0001, 0,  0.0001, -4),  
