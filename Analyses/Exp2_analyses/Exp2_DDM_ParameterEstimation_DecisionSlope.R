...
sourceCpp("Analyses\\Exp2_analyses\\Exp2_DDM_confidence_bounds_DecisionSlope.cpp") 
...
chi_square_optim <- function(params, all_observations, returnFit){  
  ...
  names(params) <- c('v1', 'v2', 'v3', 'a', 'a_slope', 'ter', 'a2_upper', 'a2_lower', 'postdriftmod', 'a2_slope_upper', 'a2_slope_lower', 'ter2')
  ...
  for (i in 1:3){
    ...
    predictions <- data.frame(DDM_confidence_bounds(v = params[v_vector[i]], a = params['a'], a_slope = params['a_slope'], ter = params['ter'], z = z, ntrials = ntrials, s = sigma, dt = dt, a2_upper = params['a2_upper'], a2_lower = params['a2_lower'], postdriftmod = params['postdriftmod'], a2_slope_upper = params['a2_slope_upper'], a2_slope_lower = params['a2_slope_lower'], ter2 = params['ter2']))
    names(predictions) <- c('rt', 'resp', 'cor', 'evidence_2', 'rtfull', 'rtconf', 'cj', 'evidence')
    
    # Transform predicted conf_evidence into cj
    
    a2_separation <- params['a2_lower'] + params['a2_upper']
    predictions$conf_evidence <- ifelse(predictions$resp == 1, predictions$evidence_2 - predictions$evidence, (-1) * predictions$evidence_2 - predictions$evidence)
...
for(i in 1:N){  # For each participant separately
  ...
  for(c in 1:4){  # For each condition separately 
    ...
    file_name <- paste0('Results\\exp2_separated_2_DecisionSlope_results_sub_', i, '_', condLab[c], '.Rdata')
    ...
    # Else, estimate parameters
      ...
      optimal_params <- DEoptim(chi_square_optim,  # Function to optimize
                                # Possible values for v (for each level of coherence: 0.1, 0.2 and 0.4), a, a_slope, ter, a2_upper, a2_lower, postdriftmod, a2_slope_upper, a2_slope_lower, ter2
                                lower = c(0, 0, 0, .5, 0,  0, 0.0001, 0.0001, 0,  0.0001,  0.0001, -4),  
                                upper = c(3, 3, 3,  4, 15, 1.5,     15,     15, 15, 15,      15,      4),
