...
// [[Rcpp::export]]
NumericMatrix DDM_confidence_bounds(double v, double a, double a_slope, double ter, double z, int ntrials, double s, double dt, double a2, double postdriftmod, double a2_slope, double ter2) {
  ...
  for (int i = 0; i < ntrials; i++) {
    ...
    if (resp == 1){
      while ((evidence < evidence_at_decision + a2/2 - t2*a2_slope) && (evidence > evidence_at_decision - a2/2 + t2*a2_slope)){
        t2 = t2 + dt;
        evidence = evidence + v_post * dt + s * sqrt(dt) * zigg.norm();
        if (evidence >= evidence_at_decision + a2/2 - t2*a2_slope){
          DATA(i,6) = 1;
        } else if (evidence <= evidence_at_decision - a2/2 + t2*a2_slope){
          DATA(i,6) = 0;
          }
      }
    } else if (resp == -1){
